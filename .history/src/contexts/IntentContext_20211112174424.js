...
const IntentContextProvider = ({ children }) => {
    ...
    const deleteIntent = async (intentId) => {
        try {
            const response = await axios.delete(`${apiUrl}/intents/${intentId}`);
            if (response.data.success)
    ...
    const findIntent = (intentId) => {
        const intent = intentState.posts.find((intent) => intent._id === intentId);
        dispatch({ type: FIND_INTENT, payload: intent });
    };

    // Update post
    const updatePost = async (updatedIntent) => {
        try {
            const response = await axios.put(
                `${apiUrl}/posts/${updatedIntent._id}`,
                updatedIntent
            );
            if (response.data.success) {
