...
const Question = () => {
    ...
    const handleAddPattern = async(pattern) => {
        ...
        const response = await axios
            .put(`${apiUrl}/intents/patterns`, newPattern)
            .then((res) => {
                console.log(res);
            })
            .catch((err) => {
        ...
        <>
            ...
            <div className="flex h-full">
                ...
                <div className="flex-1 ml-[20px] flex flex-col">
                    ...
                    <select className="block w-full mt-1 flex-1 overflow-auto" multiple onChange={handleTagChange}>
                        {tags.map((tag, index) => (
                            <option className="mb-[5px]" key={index} value={tag.tag}>{tag.name}</option>
                        ))}
                    </select>
                </div>
            </div>
