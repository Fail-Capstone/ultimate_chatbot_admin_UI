...
const Question = () => {
    ...
    const handleQuestionClick = async(id) => {
        if (selectedQuestion.includes(id)) {
            await setSelectedQuestion(selectedQuestion.filter(question => question !== id));
            console.log(selectedQuestion)
        } else {
            await loadUsersetSelectedQuestion([...selectedQuestion, id]);
            console.log(selectedQuestion)
        }
    };
        ...
        <>
            ...
            <div className="flex h-full">
                <div className="flex-1 flex">
                    <div className="bg-white flex-1 rounded-[5px] p-[10px] overflow-auto">
                        {questions.map((question, index) => (
                            <QuestionItem key={index} question={question} handleQuestion={handleQuestionClick}/>
                        ))}
                    </div>
                </div>
                <div className="flex-1 ml-[20px] flex flex-col">
                    ...
                    <select
                        className="form-multiselect block w-full mt-1 flex-1 overflow-auto"
                        multiple
                    >
